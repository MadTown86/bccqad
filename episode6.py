...
def main():
    def mini_reader():
        with open(file="episode6_output.txt", mode='r') as f:
            print(f.read())
    """
    ...
    *note that 'file' in the above syntax is the name of a local file (e.g. 'episode6_input.txt') or 
    ...
    print("\n'truncate()' - truncates the file to a certain size")
    with open("episode6_input.txt", 'r+') as f: # or 'w'
    ...
    print("\n'write()' - writes a string to the file")
    with open("episode6_output.txt", 'w') as f:
        f.write("Hello, World!")
    mini_reader()
    ...
    print("\n'json.dump()' - writes a Python object to a file in JSON format")
    with open("episode6_output.json", 'w') as f:
        json.dump({"a": 1, "b": 2, "c": 3}, f)

    # 'json.load()' - reads a JSON file and returns a Python object
    print("\n'json.load()' - reads a JSON file and returns a Python object")
    with open("episode6_output.json", 'r') as f:
        d = json.load(f)
        print(f'{d=}')
...
across, I hope.
"""
    with open("episode6_input.txt", 'w') as f:
        f.write(data)
    main()